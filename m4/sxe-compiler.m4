...
typedef __gnuc_va_list va_list;]],[[1]])], [dnl
	...
	m4_ifdef([SXE_LD_EXPORT_DYNAMIC], [SXE_LD_EXPORT_DYNAMIC])
...
AC_DEFUN([SXE_CC_MAXOPT_GNU], [dnl
	...
	if test "$sxe_cv_c_flags__fopenmp" = "yes"; then
		SXE_MSG_CHECKING([whether it is safe to use -fopenmp])
		case "$compiler_version" in
		gcc*\ 4.4.*)
			if test "$with_omp" = "yes"; then
...
dnl 		])
	...
	if test "$sxe_cv_feat_omp" = "yes" -a "$with_omp" = "yes"; then
...
AC_DEFUN([SXE_CHECK_CC], [dnl
	...
	## check for libc
	m4_ifdef([SXE_CHECK_LIBC], [SXE_CHECK_LIBC])
	m4_ifdef([SXE_CHECK_LIBC_VERSION], [SXE_CHECK_LIBC_VERSION])
